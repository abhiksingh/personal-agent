...
func (r *Registry) Register(adapter contract.Adapter) error {
	...
	if err := validateMetadata(metadata, shared.AdapterKindConnector); err != nil {
		return err
	}
	...
	if _, exists := r.adapters[metadata.ID]; exists {
		return fmt.Errorf("connector adapter already registered: %s", metadata.ID)
	}
...
func (r *Registry) SelectByCapability(capabilityKey string, preferredAdapterID string) (contract.Adapter, error) {
	...
	if preferredID != "" {
		...
		if !ok {
			return nil, fmt.Errorf("preferred connector adapter not registered: %s", preferredID)
		}
		if !supportsCapability(adapter.Metadata(), key) {
			return nil, fmt.Errorf("preferred connector adapter %s does not support capability %s", preferredID, key)
		}
		return adapter, nil
	}
	...
	if len(matches) == 0 {
		return nil, fmt.Errorf("no connector adapter supports capability: %s", key)
	}
...
func validateMetadata(metadata contract.Metadata, expectedKind shared.AdapterKind) error {
	...
	if id == "" {
		return fmt.Errorf("connector adapter metadata.id is required")
	}
	if metadata.Kind != expectedKind {
		return fmt.Errorf("connector adapter %s has invalid kind %s", id, metadata.Kind)
	}
	if len(metadata.Capabilities) == 0 {
		return fmt.Errorf("connector adapter %s must declare at least one capability", id)
	}
	...
	for _, capability := range metadata.Capabilities {
		key := strings.TrimSpace(capability.Key)
		if key == "" {
			return fmt.Errorf("connector adapter %s has capability with empty key", id)
		}
		if _, exists := seen[key]; exists {
			return fmt.Errorf("connector adapter %s declares duplicate capability %s", id, key)
		}
		seen[key] = struct{}{}
	}
	return nil
